...
load_existing_config() {
  if [[ -f "server.yaml" ]]; then
    ...
    if [[ -n "$SERVER_PORT" ]] && [[ -n "$AUTH_PASSWORD" ]]; then
...
generate_link() {
  ...
  # 节点输出链接
  cat > "$LINK_TXT" <<EOF
hysteria2://${AUTH_PASSWORD}@${ip}:${SERVER_PORT}?sni=${SNI}&alpn=${ALPN}&insecure=1#Hy2-${ip}
EOF
...
EOF
    ...
    local start_script="${work_dir}/hy2_start.sh"
    cat > "$start_script" <<EOF
#!/bin/bash
cd ${work_dir}
while true; do
  ${BIN_PATH} server -c ${work_dir}/server.yaml >/dev/null 2>&1 || true
  sleep 5
done
EOF
    chmod +x "$start_script"
    (crontab -l 2>/dev/null | grep -v "hy2_start.sh"; echo "@reboot ${start_script} &") | crontab - && \
...
cleanup_files() {
  echo "🧹 清理旧文件..."
  rm -f "$BIN_PATH" "server.yaml" "$CERT_FILE" "$KEY_FILE" "$LINK_TXT" \
